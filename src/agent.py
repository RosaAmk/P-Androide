...
class Agent():
        ...
        def apply_variation_k_tournament(self, k):
            
            if self.genomeList:
                listGenome_k = random.choices(self.genomeList, k=k)
                self.g_skill = max(listGenome_k, key=lambda i: i[1])[0] + random.gauss(0, self.sigma)
                self.g_skill = min(1, self.g_skill)
                self.g_skill = max(-1, self.g_skill)
                self.genomeList = []
            else:
